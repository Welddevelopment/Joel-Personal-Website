...
function PortalHero(){return <header className="portal-hero" id="top">
  <div className="portal-identity">
    ...
    <p>I build systems that let AI agents acquire missing capabilities, verify real outcomes, and continue their work.</p>
  ...
  <nav className="portal-grid" aria-label="Explore Joel's work">
    ...
    <a className="portal-card portal-current" href="#now"><span>Current Work</span><strong>Capability Factory</strong><small>Safe capability acquisition for AI agents.</small><b>↘</b></a>
...
export function DesignLab({direction,labMode=true}){
  ...
  return <main className={`lab lab-${direction}`} ref={root}>
    ...
    <section className="lab-proof" id="proof"><div className="lab-proof-lead"><span>THE EVIDENCE</span><strong>Built, then<br/>verified.</strong></div><a href="#now"><span>CURRENT</span><strong>Capability Factory</strong><small>Solo founder / local MVP</small></a><a href="#system"><span>SPEED</span><strong>Six days</strong><small>Idea to constrained HTTP MVP</small></a><a href="#system"><span>VERIFIED</span><strong>Real state checked</strong><small>Disposable ERPNext demo</small></a></section>
    <section className="lab-now" id="now"><div><span>CURRENT WORK</span><h2>Capability Factory.</h2><a className="lab-now-link" href="https://capability-factory-website.vercel.app" target="_blank" rel="noreferrer">Visit Capability Factory ↗</a></div><p>Capability Factory lets an AI agent diagnose a missing ability, acquire or reuse the smallest permissioned capability, verify the external result, and continue its goal.</p><p>Today it is a working local MVP for constrained HTTP APIs, tested with fictional data. I also work on Growth/GTM and growth-engineering experiments at Tsenta (YC S26).</p></section>
    <section className="lab-audit" id="system"><div className="lab-audit-mark">6</div><div className="lab-reveal"><span>DAYS FROM IDEA TO MVP</span><h2>A missing ability should not end the goal.</h2><p>The system searches trusted capabilities first, builds only the unsupported residual, acts within explicit authority, checks external state independently, and then resumes the parent goal.</p></div></section>
    ...
    <section className="lab-future"><span>WHERE THIS GOES</span><p>The immediate job is customer learning: turn the local system into one tightly scoped controlled pilot, without pretending local proof is customer proof.</p><h2>Finish GCSEs.<br/>Build the evidence.<br/><em>YC at 16.</em></h2><small>The ambition is public. The evidence still has to earn it.</small></section>
